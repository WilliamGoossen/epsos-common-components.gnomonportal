...
<%@ page import="gnomon.hibernate.model.crm.CrMeetingComment" %>
...
<h3><%= LanguageUtil.get(pageContext, "crm.meeting.comment.list") %></h3>

<form name="CRM_TASK_Comments_ButtonForm" action="/some/url" method="post">
<display:table id="comment" name="commentsList" requestURI="//ext/crm/meetings/listComments" pagesize="10" sort="list" style="font-weight: normal; width: 100%; border-spacing: 0">
<% CrMeetingComment gnItem = (CrMeetingComment) pageContext.getAttribute("comment"); %>
<display:column titleKey="crm.meeting.comment.creatorid" sortable="true">
<%
...
<display:column titleKey="crm.meeting.comment.entryDate" sortable="true">
<%= CrMeetingForm.date_time_format.format(gnItem.getEntryDate()) %>
</display:column>
<display:column property="subject" titleKey="crm.meeting.comment.subject" sortable="true"/>
<display:column property="description" titleKey="crm.meeting.comment.description" sortable="true"/>
	...
	<tr>
		...
		<td>
			<a href="<portlet:actionURL>
					<portlet:param name="struts_action" value="/ext/crm/meetings/loadComment"/>
					<portlet:param name="mainid" value="<%= gnItem.getMainid().toString() %>"/>
					<portlet:param name="loadaction" value="edit"/>
					<portlet:param name="meetingid" value="<%= meetingid.toString() %>"/>
...
<% if (hasAdd) {%>
	<tiles:insert page="/html/portlet/ext/struts_includes/button.jsp" flush="true">
	  <tiles:put name="action"  value="/ext/crm/meetings/loadComment" />
	  <tiles:put name="buttonName" value="addButtonMeetingComment" />
	  ...
	  <tiles:putList name="actionParamList">
	  	<tiles:add value="meetingid"/>
	  	<tiles:add value="redirect"/>
	  </tiles:putList>
	  <tiles:putList name="actionParamValueList">
	  	<tiles:add><%= meetingid.toString() %></tiles:add>
...
<a href="<%= redirect %>"><%= LanguageUtil.get(pageContext, "crm.button.back") %></a>
<% } else { %>
