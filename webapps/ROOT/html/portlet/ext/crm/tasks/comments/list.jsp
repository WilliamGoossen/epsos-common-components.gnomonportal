...
<%@ page import="gnomon.hibernate.model.crm.CrTaskComment" %>
...
<h3><%= LanguageUtil.get(pageContext, "crm.task.comment.list") %></h3>

<form name="CRM_TASK_Comments_ButtonForm" action="/some/url" method="post">
<display:table id="comment" name="commentsList" requestURI="//ext/crm/tasks/listComments" pagesize="10" sort="list" style="font-weight: normal; width: 100%; border-spacing: 0">
<% CrTaskComment gnItem = (CrTaskComment) pageContext.getAttribute("comment"); %>
<display:column titleKey="crm.task.comment.creatorid" sortable="true">
<%
...
<display:column titleKey="crm.task.comment.entryDate" sortable="true">
<%= CrTaskForm.date_time_format.format(gnItem.getEntryDate()) %>
</display:column>
<display:column property="subject" titleKey="crm.task.comment.subject" sortable="true"/>
<display:column property="description" titleKey="crm.task.comment.description" sortable="true"/>
	...
	<tr>
		...
		<td>
			<a href="<portlet:actionURL>
					<portlet:param name="struts_action" value="/ext/crm/tasks/loadComment"/>
					<portlet:param name="mainid" value="<%= gnItem.getMainid().toString() %>"/>
					<portlet:param name="loadaction" value="edit"/>
					<portlet:param name="taskid" value="<%= taskid.toString() %>"/>
...
<% if (hasAdd) {%>
	<tiles:insert page="/html/portlet/ext/struts_includes/button.jsp" flush="true">
	  <tiles:put name="action"  value="/ext/crm/tasks/loadComment" />
	  <tiles:put name="buttonName" value="addButtonTaskComment" />
	  ...
	  <tiles:putList name="actionParamList">
	  	<tiles:add value="taskid"/>
	  	<tiles:add value="redirect"/>
	  </tiles:putList>
	  <tiles:putList name="actionParamValueList">
	  	<tiles:add><%= taskid.toString() %></tiles:add>
