...
<c:choose>
	...
	<c:when test='<%= schemaTab.equals("show-selected") %>'>
		...
		<br>
		<input class="portlet-form-button" type="button" value="<bean:message key="save" />" onClick="document.<portlet:namespace />fm.<portlet:namespace />sourceItems.value = listSelect(document.<portlet:namespace />fm.<portlet:namespace />current_banners); submitForm(document.<portlet:namespace />fm);">
	</c:when>
</c:choose>
