...
<!-- Attachments List -->
<display:table id="attachment" name="attachments" requestURI="//ext/smslists/loadPost?actionURL=true" pagesize="5" sort="list" export="false" style="font-weight: normal; width: 100%; border-spacing: 0">
...
<c:choose>
	<c:when test="<%=gnItem.getField2()!=null%>">
		...
		<a target="_blank" href="<portlet:actionURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"><portlet:param name="struts_action" value="/ext/smslists/getPostFile"/><portlet:param name="mainid" value="<%= gnItem.getMainid().toString() %>"/><portlet:param name="loadaction" value="view"/></portlet:actionURL>"><%= gnItem.getField2().toString() %></a>
		</img>
	</c:when>
	<c:otherwise>
				...
				<tr>
					...
					<td>
						<a href="<portlet:actionURL>
									<portlet:param name="struts_action" value="/ext/smslists/loadPostFile"/>
...
<br/>
<form name="BsSmsListPostFileForm" action="/ext/smslists/loadPostFile" method="post">
<tiles:insert page="/html/portlet/ext/struts_includes/button.jsp" flush="true">
	<tiles:put name="action"  value="/ext/smslists/loadPostFile" />
	<tiles:put name="buttonName" value="addButton" />
	<tiles:put name="buttonValue" value="add-attachment" />
	<tiles:put name="formName"   value="BsSmsListPostFileForm" />
...
<%
	...
	params.put("mainid", request.getParameter("smslistid"));
	params.put("loadaction", "view");

	pageContext.setAttribute("paramsName", params);
%>
<br/>
<html:link styleClass="beta1" action="/ext/smslists/load" name="paramsName">
