...
<display:table id="new" name="news" requestURI="//ext/news/list?actionURL=true" pagesize="10" sort="list" export="false" style="width: 100%;">
    ...
    <display:column titleKey="title" sortable="false" style="width:100%">
	
    <div class="date">
    	<%= gnomon.business.DateUtils.convertDate(StringUtils.check_null(gnItem.getField2(),""),"yyyy-MM-dd HH:mm:ss","dd-MM-yyyy")%>
    </div>

    	<%//= gnItem.getField2().toString() %>

    <div class="title"><a href="<portlet:actionURL>
			<portlet:param name="struts_action" value="/ext/news/load"/>
			<portlet:param name="mainid" value="<%= gnItem.getMainid().toString() %>"/>
			<portlet:param name="loadaction" value="view"/>
			<portlet:param name="redirect" value="<%=currentURL%>"/>
			</portlet:actionURL>"><%= gnItem.getField4().toString() %></a><br>
	<%//=StringUtils.subStr(StringUtils.stripHTML(StringUtils.check_null(gnItem.getField5(),"")).trim(),300)%>
    </div>
	</display:column>
