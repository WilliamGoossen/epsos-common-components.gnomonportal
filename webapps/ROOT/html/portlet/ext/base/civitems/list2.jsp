...
<!-- CivItems List -->
<display:table id="civitem" name="civitems" requestURI="//ext/civitems/list?actionURL=true" sort="list" export="false" style="font-weight: normal; width: 100%; border-spacing: 0">
...
<a href="<portlet:actionURL>
		...
		<c:if test="<%=gnItem.getField4()!=null%>">
			<img src="<%="/FILESYSTEM/" + com.liferay.portal.util.PortalUtil.getCompanyId(request) + "/" + filePath + /* gnItem.getMainid() + */ "/" + gnomon.business.GeneralUtils.createThumbnailPath((String)gnItem.getField4())%>" alt="<%= gnItem.getField6().toString() %>" align="left" />
		</c:if>
		<%= gnItem.getField6().toString() %>
        </a></strong>
<br/>
<%=StringUtils.subStr(StringUtils.stripHTML(StringUtils.check_null(gnItem.getField7(),"")).trim(),200)%>
</display:column>
...
<% if (PortletPermissionUtil.contains(permissionChecker, plid, "bs_civitem", "add")) { %>
...
<% if (user != null && user.getLogin() != null && user.getLogin().equals("root")) { %>
<form name="BS_CIV_ITEM_IMPORT" method="post" action="<portlet:actionURL><portlet:param name="struts_action" value="/ext/civitems/import" /></portlet:actionURL>" enctype="multipart/form-data">
<fieldset>
<legend>Import Items from Excel file</legend>
	<span><%= LanguageUtil.get(pageContext, "bs.civitems.metadata.type") %></span>
	<select name="metadataClassId">
		<option name="" value=""></option>
	<%
	...
	<span><%= LanguageUtil.get(pageContext, "bs.civitems.attachmentFile") %></span>
	<input title="Enter Excel file to Import"  type="file" name="excelFile">
