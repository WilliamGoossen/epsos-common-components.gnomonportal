...
<div id="latestcontent-tile">


<ul>
	<% for (ViewResult view: viewList) { %>
        <li>
		<!--<% if (view.getField11() != null && ((Boolean)view.getField11()).booleanValue()) { %>
		...
		<% } %>--><strong>
                ...
                <% 
			  		...
			  		if (imageName != null && showImages) { %>
			  		    <img src="<%="/FILESYSTEM/" + com.liferay.portal.util.PortalUtil.getCompanyId(request) + "/" + filePath + view.getMainid() + "/" + gnomon.business.GeneralUtils.createThumbnailPath((String)view.getField9())%>" alt="<%= view.getField7().toString() %>" align="center" />
			  		<% } %>
                    <br />
	...
	out.print("title=\"" +LanguageUtil.get(pageContext, "bs.events.cancelled") + "\" style=\"text-decoration: line-through\" ");
					...
					<portlet:param name="redirect" value="/web/guest/events"/>
			  		...
			  		<%= date_format.format((java.util.Date)view.getField4()) %>
                    <br />
