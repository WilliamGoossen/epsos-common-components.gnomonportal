		  	...
		  	<li>
				...
				<% if (rootPlid <= 0) { %>
			  	<a href="<liferay-portlet:renderURL portletName="bs_news" windowState="<%=WindowState.NORMAL.toString()%>">
					...
					<span class="date"><%= gnomon.business.DateUtils.convertDate(StringUtils.check_null(view.getField3(),""),"yyyy-MM-dd HH:mm:ss","dd-MM-yyyy")%>, <span class="title"><%=StringUtils.check_null(view.getField4(),"")%></span>
					</span>
				 </a>
				 <% } else { 
				 ...
				 <%} %>
					<span class="shortdescription"><%= StringUtils.check_null(view.getField5(),"") %></span>
