	...
	<% for (ViewResult view: viewList) { %>
		  	...
		  	<li>
		  		...
		  		if (imageName != null && showImages) { %>
		  			<img src="/<%=filePath + view.getMainid() + "/" + gnomon.business.GeneralUtils.createThumbnailPath(imageName) %>" alt="news-img" align="left" />
		  		<% } %>
				...
				<% if (rootPlid <= 0) { %>
			  	<a href="<liferay-portlet:renderURL portletName="bs_news" windowState="<%=WindowState.NORMAL.toString()%>">
					...
					</liferay-portlet:renderURL>"><span class="date"><%= gnomon.business.DateUtils.convertDate(StringUtils.check_null(view.getField3(),""),"yyyy-MM-dd HH:mm:ss","dd-MM-yyyy")%></span> <span class="title"><%=StringUtils.check_null(view.getField4(),"")%></span>
				</a>
				<% } else { 
				...
				<span class="date"><%= gnomon.business.DateUtils.convertDate(StringUtils.check_null(view.getField3(),""),"yyyy-MM-dd HH:mm:ss","dd-MM-yyyy")%></span> <span class="title"><%=StringUtils.check_null(view.getField4(),"")%></span>
				</a>
				<% } %>
            </li>
	<% } %>
