...
<!-- Faq List -->
<display:table id="faq" name="faqs" requestURI="//ext/faqs/list?actionURL=true" pagesize="5" sort="list" export="false" style="font-weight: normal; width: 100%; border-spacing: 0">
	<% ViewResult gnItem = (ViewResult) pageContext.getAttribute("faq"); %>
	...
	<div id="browse:actionsMenu_1_<%=gnItem.getMainid().toString()%>" style="position: absolute; display: none; padding-left: 2px;">
		...
		<c:if test="<%= hasDelete %>">
			<tr>
				...
				<td>
					<a href="<portlet:actionURL><portlet:param name="struts_action" value="/ext/faq/load"/>
							<portlet:param name="mainid" value="<%= gnItem.getMainid().toString() %>"/>
			...
			<tr>
				...
				<td>
					<a href="<portlet:actionURL>
							<portlet:param name="struts_action" value="/ext/faq/listContactPersons"/>
							<portlet:param name="faqId" value="<%= gnItem.getMainid().toString() %>"/>
							<portlet:param name="redirect" value="<%=currentURL%>"/>
							</portlet:actionURL>">
