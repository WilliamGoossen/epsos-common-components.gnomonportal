...
<script type="text/javascript">
	...
	<%if ( inOneRow==false && remainCols==0) { %>
		<tr>
	<%}%>
<%String onDeactivateEH = formFieldName + "_saveArticle();";
//		onSubmitBody1 += onDeactivateEH + "\n";%>
	...
	<%--	    <INPUT type="hidden" name="<portlet:namespace/><%=formFieldName%>" value="<%=formValue %>">	
		<span class="gamma1-FormText"><%= LanguageUtil.get(pageContext, fieldTitle) %></span></td><td>
		 <IFRAME  ondeactivate = "<%=onDeactivateEH%> "  id=<portlet:namespace/><%=formFieldName%>_editor name=<portlet:namespace/><%=formFieldName%>_editor src="<%= contextPath %>/html/js/editor/editor.html" frameBorder=0  width="100%" height=400 ></IFRAME>
--%>
		 <tr>
	     	<td><span class="gamma1-FormText" ><%= LanguageUtil.get(pageContext, formFieldName) %></span></td>
	     	<td>
	     	<textarea name="<%=formFieldName%>" cols="50" rows="15"  mce_editable="true">
		     	<%=formValue%>
	     	</textarea>
	 </tr> 
...
<%if ((inOneRow==false && remainCols==pageColumns-1) || index ==fields.size()-1) { %>
	...
	<script type="text/javascript">
	// For the textareahtml to work in Mozilla Browsers
	function formOnSubmit1() {
