...
<display:table id="photo" name="productPhotos" requestURI="//ext/products/admin/listProductPhotos?actionURL=true" sort="list" export="false" style="font-weight: normal; width: 100%; border-spacing: 0">
<% ViewResult gnItem = (ViewResult) pageContext.getAttribute("photo"); %>

<display:column titleKey="description" sortable="false">
<%
...
productid + "/" + gnomon.business.GeneralUtils.createThumbnailPath((String)gnItem.getField2()); 

String realImageFilePath = "/FILESYSTEM/" + PortalUtil.getCompanyId(request) + "/" + 
GetterUtil.getString(PropsUtil.get("ecommerce.product.store"), CommonDefs.DEFAULT_STORE_PATH) + 
productid + "/" + (String)gnItem.getField2(); 
%>

<a href="<%= realImageFilePath %>" class="thickbox">
<img align="left" border="0" src="<%= thumbnailImageFilePath %>">
</a>
<%= gnItem.getField1() != null ? gnItem.getField1().toString() : ""%>
</display:column>
		...
		<tr>
			...
			<td>
				<a href="<portlet:actionURL>
						<portlet:param name="struts_action" value="/ext/products/admin/loadProductPhoto"/>
