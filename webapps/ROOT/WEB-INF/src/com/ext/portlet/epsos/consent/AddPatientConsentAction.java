...
public class AddPatientConsentAction extends GnProcessAction {
	...
	protected void processActionLocal(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (patientResult != null && patientResult.size() > 0)
			{
				...
				PatientConsentObject consentObject = new PatientConsentObject();
				consentForm.populateObjectFromForm(consentObject);
				
				epsosHelper.addPatientConsent(consentObject, patient, CommonUtil.getHttpServletRequest(req));
